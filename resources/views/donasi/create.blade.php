...
<head>
    ...
    <style>
        ...
        .field input[type="text"], .field input[type="number"], .field textarea, .field input[type="file"] { width:100%; padding:8px 10px; border:1px solid #ddddddff; border-radius:4px; box-sizing:border-box }
    ...
    <div class="form-container">
        ...
        <form method="POST" action="/donasi" enctype="multipart/form-data">
            ...
            <div class="field">
                ...
                <input type="text" id="nama_donatur" name="nama_donatur" value="{{ old('nama_donatur') }}" required maxlength="255">
            </div>

            <div class="field">
                <label for="jenis_donasi">Jenis Donasi</label>
                <input type="text" id="jenis_donasi" name="jenis_donasi" value="{{ old('jenis_donasi') }}" required maxlength="255">
            </div>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                const fields = [
                    ...
                    {id: 'gambar', message: 'Silakan unggah gambar.'}
                ];

                fields.forEach(function(f) {
                    const el = document.getElementById(f.id);
                    if (!el) return;
                    // Clear custom message when user types/selects
                    el.addEventListener('input', function() { el.setCustomValidity(''); });
                    // For file inputs, also clear on change
                    el.addEventListener('change', function() { el.setCustomValidity(''); });

                            // Provide custom message when invalid
                            el.addEventListener('invalid', function() {
                                const isEmptyFile = el.type === 'file' && el.files && el.files.length === 0;
                                // valueMissing or empty file
                                if (el.validity.valueMissing || isEmptyFile) {
                                    el.setCustomValidity(f.message);
                                }
                                // badInput occurs when a number field contains non-numeric text
